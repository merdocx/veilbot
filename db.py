...
def migrate_extend_payments_schema():
    ...
    try:
        ...
        # Требуемые дополнительные поля по новой схеме (subscription_id нужен до migrate_add_common_indexes)
        required_columns = [
...
def migrate_add_common_indexes():
    ...
    # cursor.execute("CREATE INDEX IF NOT EXISTS idx_keys_expiry_at ON keys(expiry_at)")
        ...
        # Индекс для запросов по subscription_id (подписка -> платежи, расхождения)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_payments_subscription_id ON payments(subscription_id) WHERE subscription_id IS NOT NULL")
...
def migrate_add_access_level_to_servers():
    """Добавление поля access_level в servers для управления уровнями доступа (all, paid, vip)"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
    try:
        ...
        if 'access_level' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN access_level TEXT DEFAULT 'all'")
            conn.commit()
            logging.info("Поле access_level добавлено в servers")
            
            # Заполняем существующие записи на основе available_for_purchase
            # Если available_for_purchase = 1, то access_level = 'all', иначе 'paid'
            cursor.execute("""
                UPDATE servers 
                SET access_level = CASE 
                    WHEN available_for_purchase = 1 THEN 'all'
                    ELSE 'paid'
                END
            """)
            conn.commit()
            logging.info("Существующие записи заполнены значениями access_level на основе available_for_purchase")
        else:
            logging.info("Поле access_level уже существует в servers")
    except sqlite3.OperationalError as e:
        logging.error(f"Ошибка миграции добавления access_level: {e}", exc_info=True)
    finally:
        conn.close()
...
def migrate_add_subscription_traffic_limits():
    ...
    try:
        ...
        if 'last_traffic_reset_at' not in columns:
            cursor.execute("ALTER TABLE subscriptions ADD COLUMN last_traffic_reset_at INTEGER")
            conn.commit()
            logging.info("Поле last_traffic_reset_at добавлено в subscriptions")
