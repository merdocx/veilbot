    ...
    window.initLiveSearch = function(config) {
        ...
        setTimeout(() => {
            ...
            const performSearch = () => {
                ...
                // Формируем URL с параметрами поиска (все поля формы: q, paid, inactive, page и т.д.)
                const formData = new FormData(searchForm);
                const params = new URLSearchParams(formData);
                const url = `${pageUrl}?${params.toString()}`;

                // Обновляем URL в адресной строке, чтобы фильтры сохранялись при обновлении страницы
                try {
                    window.history.pushState({}, '', url);
                } catch (e) {
                    // Игнорируем ошибки pushState (например, в старых браузерах)
                }
