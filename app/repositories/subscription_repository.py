...
class SubscriptionRepository:
    ...
    def extend_subscription_by_duration(self, subscription_id: int, duration_sec: int, tariff_id: Optional[int] = None, max_expires_at: Optional[int] = None) -> int:
        ...
        Args:
            subscription_id: ID подписки
            duration_sec: Количество секунд для продления
            tariff_id: Опционально - обновить tariff_id подписки. Если None, сохраняется текущее значение.
            max_expires_at: Максимальное значение expires_at (для защиты от ручных установок). Если None, ограничений нет.
            
        Returns:
            Новое значение expires_at после продления
        """
        with open_connection(self.db_path) as conn:
            c = conn.cursor()
            now = int(time.time())
    ...
    async def extend_subscription_by_duration_async(self, subscription_id: int, duration_sec: int, tariff_id: Optional[int] = None, max_expires_at: Optional[int] = None) -> int:
        ...
        async with open_async_connection(self.db_path) as conn:
            ...
            # Получаем новое значение expires_at
            async with conn.execute("SELECT expires_at FROM subscriptions WHERE id = ?", (subscription_id,)) as check_cursor:
    ...
    def list_subscriptions(
        ...
        """Получить список подписок с информацией о ключах.
        ...
        Args:
            ...
            include_inactive: Если True, включать подписки с is_active = 0 (по умолчанию только активные).
        """
        with open_connection(self.db_path) as conn:
            c = conn.cursor()
            ...
            if paid_only:
                ...
                """
            # По умолчанию только активные; при include_inactive показываем все
            active_condition = "" if include_inactive else "AND s.is_active = 1"
    ...
    def count_subscriptions(
        ...
        """Получить количество подписок (по умолчанию только активных, при include_inactive — всех).

        Args:
            query: Поисковый запрос
            paid_only: Если True, считать только платные подписки (price_rub > 0, не VIP)
            include_inactive: Если True, учитывать подписки с is_active = 0.
        """
        with open_connection(self.db_path) as conn:
            c = conn.cursor()
            ...
            if query:
                ...
                if paid_only:
                    ...
                    sql = """
                    SELECT COUNT(*) FROM subscriptions s WHERE 1=1 {active_condition}
                    """
                sql = sql.format(active_condition=active_condition)
                c.execute(sql)
            row = c.fetchone()
            return row[0] if row else 0

    def get_subscription_filter_stats(
        self,
        query: Optional[str] = None,
        paid_only: bool = False,
        include_inactive: bool = False,
        ...
        paid_condition = ""
        if paid_only:
            paid_condition = " AND t.price_rub > 0 AND (u.is_vip IS NULL OR u.is_vip = 0)"
        active_condition = "" if include_inactive else " AND s.is_active = 1"
        extra_active = " AND s.expires_at > ?"
        if include_inactive:
            extra_active = " AND s.is_active = 1 AND s.expires_at > ?"

        with open_connection(self.db_path) as conn:
            c = conn.cursor()
            if query:
                like = f"%{query.strip()}%"
                base_where = """((CAST(s.id AS TEXT) LIKE ?
                    OR CAST(s.user_id AS TEXT) LIKE ?
                    OR s.subscription_token LIKE ?
                    OR t.name LIKE ?
                ...
                active_count = int((c.fetchone() or (0,))[0])
                c.execute(
                    """SELECT COUNT(DISTINCT s.id) FROM subscriptions s
                    LEFT JOIN tariffs t ON s.tariff_id = t.id LEFT JOIN users u ON s.user_id = u.user_id
                    LEFT JOIN payments p ON s.id = p.subscription_id AND p.status = 'completed'
                    WHERE """ + base_where + extra_active + """ AND s.expires_at < ?
                    AND (p.id IS NOT NULL OR (t.price_rub IS NOT NULL AND t.price_rub > 0))""",
    ...
    def get_subscription_traffic_limits_batch(self, subscription_ids: list[int]) -> dict[int, int]:
        """Получить лимиты трафика (в байтах) для списка подписок одним запросом.
        Логика: COALESCE(s.traffic_limit_mb, t.traffic_limit_mb, 0) * 1024*1024.
        Returns: dict[subscription_id, limit_bytes]"""
        if not subscription_ids:
            return {}
        with open_connection(self.db_path) as conn:
            c = conn.cursor()
            placeholders = ','.join('?' * len(subscription_ids))
            c.execute(f"""
                SELECT s.id, (COALESCE(s.traffic_limit_mb, t.traffic_limit_mb, 0) * 1024 * 1024)
                FROM subscriptions s
                LEFT JOIN tariffs t ON s.tariff_id = t.id
                WHERE s.id IN ({placeholders})
            """, subscription_ids)
            return {int(row[0]): int(row[1] or 0) for row in c.fetchall()}
