...
class TestPaymentIntegration:
    ...
    async def test_payment_filtering_integration(self, payment_service, payment_repo):
        ...
        payments.append(paid_payment)
        
        # Создаем платежи в БД
        for payment in payments:
            await payment_repo.create(payment)
        
        # Act
        user_payments = await payment_service.get_user_payments(123456789)
        pending_payments = await payment_service.get_pending_payments()
        
        # Assert
        assert len(user_payments) >= 4  # Может быть больше из-за существующих данных
        assert len(pending_payments) >= 3  # Три ожидающих платежа
    ...
    async def test_cleanup_expired_payments_integration(self, payment_service):
