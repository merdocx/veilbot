...
### 8.1a Разовые и миграционные скрипты (`scripts/archive/`)

Исторические и инцидентные скрипты (проверка конкретных подписок, ручное обслуживание БД и т.п.) перенесены в **`scripts/archive/`**. Перед запуском убедитесь, что сценарий актуален для текущей схемы БД; при сомнениях сделайте бэкап `vpn.db`. Пример: обслуживание SQLite — `scripts/archive/db_maintenance.py` (ручной запуск, не часть рантайма). Дополнительно в архиве: `fix_subscription_traffic_after_payment.py`, `fix_subscriptions_316_322.py`, `fix_subscriptions_478_479_480.py` (инцидентные правки подписок).


### 8.2 Массовая рассылка (`docs/archive/broadcast.py`)

**Назначение**: Исторический скрипт массовой рассылки; не входит в рантайм. Перед использованием сверьте код с текущим API бота.

**Использование**:
```bash
cd /root/veilbot
# Отредактировать текст в docs/archive/broadcast.py при необходимости
python3 docs/archive/broadcast.py
```
...
1. Откройте `docs/archive/broadcast.py`
2. Найдите переменную с текстом рассылки (см. файл)
3. Измените текст сообщения
4. Сохраните и запустите скрипт

**Пример использования**:
```python
# В docs/archive/broadcast.py
