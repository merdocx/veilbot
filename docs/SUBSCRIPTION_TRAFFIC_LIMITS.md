...
- Лимиты хранятся в тарифах (`tariffs.traffic_limit_mb`), не дублируются в `subscriptions`
...
1. **Лимит берется из тарифа:** `tariffs.traffic_limit_mb` → используется для подписки
